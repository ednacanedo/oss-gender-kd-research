...
```{r, echo=FALSE}
t2 <- sqldf("select full_name, count(*) totalTF 
       from tf_ds
       group by full_name 
       having totalTF >= 5 
       order by 2 desc")
...
```{r}
distinct_tfs <- sqldf("select user, login, count(*) total
                       from tf_ds 
                       group by full_name, login 
                       order by 3 desc")
...
sqldf("select gender_final, count(*) total from t_all_commiters group by gender_final")

sqldf("select gender_final, count(*) total from t_tf group by gender_final")

slices <- c(14220, 8897, 122953)
